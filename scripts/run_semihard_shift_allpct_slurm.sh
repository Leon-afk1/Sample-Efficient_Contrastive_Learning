#!/usr/bin/env bash
#SBATCH --job-name=har_semihard_shift_allpct
...
#SBATCH --output=Log/slurm/semihard_shift_%A_%a.out
#SBATCH --error=Log/slurm/semihard_shift_%A_%a.err
...
echo "==========================================" | tee -a "$LOG_FILE"
echo "SEMIHARD+SHIFT MINING — $PCT_LABEL" | tee -a "$LOG_FILE"
echo "==========================================" | tee -a "$LOG_FILE"
